...
export function MediaLibrary() {
  ...
  return (
    <section className="mt-16">
      <div className="mx-auto mb-6 flex w-[min(1200px,95%)] items-end justify-between">
        <div>
      ...
      <div className="space-y-8">
        {Object.entries(media).map(([cat, items]) => (
          <div key={cat}>
            <h3 className="mx-auto mb-3 w-[min(1200px,95%)] font-display text-lg font-semibold text-foreground">{cat}</h3>
            <div className="flex snap-x snap-mandatory gap-3 overflow-x-auto px-4 pb-4 sm:gap-4 sm:px-8">
              {items.map((m) => {
                const Icon = iconFor[m.type];
                return (
                  <button key={m.id} onClick={() => setSelected(m)} className="group relative aspect-[3/4] w-40 shrink-0 snap-start overflow-hidden rounded-2xl shadow-soft transition hover:scale-[1.03] sm:w-52">
                    ...
                    <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/70 to-transparent p-3 text-left">
                      ...
                      <div className="mt-1 font-display text-sm font-semibold leading-tight text-white line-clamp-2">{m.title}</div>
                    </div>
                  </button>
                );
              })}
